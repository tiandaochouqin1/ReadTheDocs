...
.. admonition:: 摘要
   ...
   Linux 内核编译实战笔记：源码获取、defconfig/menuconfig 配置、交叉编译工具链选择、内核模块编译。适合第一次编译内核或需要交叉编译 ARM/嵌入式内核的开发者。
...
2. 在源码树外部编译：（OSC中的Lab，在ubuntu18中会提示insmod签名问题）
   ...
   :download:`kernel_module.c </files/Kernel_compilation/kernel_module.c>`
