...
.. admonition:: 摘要
   ...
   动态链接的完整知识体系：PLT/GOT 延迟绑定、``ld.so`` 加载流程、符号解析与重定位、动态/静态链接的优劣对比。适合需要深入理解 ELF 加载和链接器行为的系统/逆向开发者。
...
.. figure:: /images/DynamicLink/dl_runtime_resolve.png
   :scale: 50%
...
1. **PLT/GOT 延迟绑定** — 首次调用外部函数时通过 PLT 跳转到 ``ld.so`` 解析符号地址并写入 GOT，后续调用直接通过 GOT 跳转。这是动态链接的性能关键。
2. **符号优先级** — ``.a/.so + .a/.so``：按链接顺序取第一个符号，不区分强弱；``.o + .o``：只考虑强弱，强符号覆盖弱符号。这个差异是很多链接错误的根源。
3. **visibility 的实际价值** — 将内部符号标记为 hidden 可以减少 5%-20% 的 DSO 体积、加快加载速度、减少 PLT 重定向开销。推荐编译时 ``-fvisibility=hidden`` + 需要导出的函数显式标记 ``__attribute__((visibility("default")))``。
...
.. seealso::
   ...
   `main之前 <../Basic/C/main.rst>`_
      ``_start`` 到 ``__libc_start_main`` 的启动流程。
