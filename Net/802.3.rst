...
.. admonition:: 摘要
   ...
   IEEE 802.3 以太网标准的硬件手册式笔记：MAC 层功能、SGMII/GMII 接口、自协商状态机（Clause 22/37）以及车载以太网和 InfiniBand 对比。适合嵌入式/SoC 开发者和 PHY 驱动工程师。


参考文档
===========
1. :download:`IEEE802.3-2018 </files/802.3/IEEE802.3-2018.rar>`
2. xlinx的手册
3. 以太网权威指南(第二版)/ :download:`Ethernet: The Definitive Guide 2nd </books/802.3/Ethernet The Definitive Guide, 2nd Edition.pdf>`
...
.. figure:: /images/802.3/FLP_Burst.png
...
6. `Clause_37_Auto-Negotiation <https://www.iol.unh.edu/sites/default/files/knowledgebase/ge/Clause_37_Auto-Negotiation.pdf>`__
   :download:`Clause_37_Auto-Negotiation.pdf </files/802.3/Clause_37_Auto-Negotiation.pdf>`
...
.. figure:: /images/802.3/AN_state_diagram.jpg
...
1. :download:`Serial-GMII Specification </files/802.3/SGMII.pdf>`
...
.. figure:: /images/802.3/ethernet_bus.jpg
...
.. code-block:: bash
   ...
   # 查看自协商结果（speed、duplex、link partner capabilities）
   ethtool eth0 | grep -E "Speed|Duplex|Auto-negotiation|Link partner"

   # 查看 PHY 寄存器（需要驱动支持）
   ethtool -d eth0

   # mii-tool 用于老式 MII 接口
   mii-tool -v eth0
...
.. warning::
   ...
   强制模式关闭自协商后，对端如果保持自协商模式，会进入并行检测，只能协商为半双工。这会导致严重的双工不匹配问题（一端全双工、一端半双工），丢包率急剧上升，是最常见的网络故障之一。
...
.. code-block:: bash
   ...
   # 查看收发统计和错误计数
   ethtool -S eth0 | grep -E "err|drop|crc|collision"

   # 查看网卡的硬件 offload 功能状态
   ethtool -k eth0
...
1. **自协商是 1000M 的必选项** — 1000BASE-T 标准强制要求自协商，用于协商主从时钟和双工模式。强制 1000M 实际上不合规。
2. **SGMII vs SerDes** — SGMII 接口可以通过配置寄存器直接切换为 SerDes 模式接光模块，不需要 PHY 芯片。区别在于 SGMII 插入了控制信息（1.25Gbps 中只有 1Gbps 是有效数据），而 SerDes 通过 8B/10B 编码提升线速率。
3. **FLP 的巧妙设计** — 16bit base page 用 33 个脉冲位（17 个时钟 + 16 个数据）承载，周期 16±8ms。即使对端不支持自协商，并行检测也能通过 NLP（10M）或 4B/5B Idle（100M）识别速率。

.. seealso::

   `PHY <./Phy.rst>`_
      PHY 芯片的寄存器、MDIO 总线和 Linux PHY 驱动架构。

   `ARP <./Protocols/arp.rst>`_
      链路层之上的邻居子系统——地址解析与可达性管理。
